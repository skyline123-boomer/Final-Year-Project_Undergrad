...
val_db = val_db.map(preprocess).batch(batchsz2)
...
def main():
    ...
    plt.bar(X, correct_list, 0.4, color="green")
    plt.xlabel("Types of failed array elements")
    plt.ylabel("Correct number of samples")
    for x, y in zip(X, correct_list):
