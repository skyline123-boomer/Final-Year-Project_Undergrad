...
def main():
    ...
    for epoch in range(51):
        ...
        acc = total_correct / total_number
        train_acc_list2.append(acc)


    plt.plot(train_acc_list1)
    plt.plot(train_acc_list2)
    plt.title('Model Train Accurancy')
    plt.ylabel('Accurancy')
    plt.xlabel('Epoch')
    plt.legend(['DNN', 'Transferred Model'], loc='lower right')
    plt.grid()
    plt.show()
